  ...
  window.startDemo = function(cb) {
    ...
    stateEls.minified.addEventListener('change', function(event) {
      ...
      reloadScripts(urls, function() {
        ...
        const vhs = window.videojs.Vhs
        // vhs.options_.experimentalLLHLS = true
        const customTagParsers = [
          {
            expression: /#EXT-X-SOAPBOX-START-TIME/,
            customType: 'soapboxStartTime',
            segment: false
          },
          {
            expression: /#EXT-X-SOAPBOX-SEGMENT-START-TIME/,
            customType: 'soapboxSegmentStartTime',
            segment: true
          },
          {
            expression: /#EXT-X-SOAPBOX-META/,
            customType: 'soapboxMeta',
            segment: true
          }
        ];
        ...
        const playlistLoader = new vhs.PlaylistLoader(stateEls.url.value, fakeVhs, { })
        window.playlistLoader = playlistLoader
        const logEvents = ['loadedplaylist', 'loadedmetadata', 'mediaupdatetimeout', 'error', 'playlistunchanged', 'dispose', 'mediachanging']
        for (const event of logEvents) {
          playlistLoader.on(event, (...args) => {
            console.debug(event, ...args)
          })
        }
        // window.videojs.log.level('all')
